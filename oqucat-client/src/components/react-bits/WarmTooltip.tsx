...
type TriggerProps = Required<
  Omit<WarmTooltipProps, 'delay' | 'warmWindow' | 'shortcut'>
> &
...
const SIGN: Record<WarmTooltipSide, number> = {
  top: 1,
  bottom: -1,
  left: -1,
  right: 1,
...
const horizontal = (side: WarmTooltipSide) =>
...
export const WarmTooltipGroup = forwardRef<
  WarmTooltipGroupHandle,
...
>(function WarmTooltipGroup(
  ...
  const leanUnit = useSpring(
    useTransform(speed, [-FULL_LEAN_SPEED, 0, FULL_LEAN_SPEED], [1, 0, -1], {
      clamp: true,
    }),
    LEAN_SPRING
  )
  ...
  const arrowAt = useTransform(
    ...
    ([x, y, width, height]: number[]) => {
      ...
      return horizontal(side)
        ? clamp(x - X, 10, width - 10)
        : clamp(y - Y, 10, height - 10)
    }
  )
  ...
  const isWarm = () =>
    st.current.state !== 'closed' || now() < st.current.warmUntil
  const notify = () => {
    const next = isWarm()
  ...
  const group = useMemo(
    () => ({
      ...
      hide: (tooltipId: string, instant?: boolean) =>
                ...
                <AnimatePresence initial={false} custom={st.current.swap}>
                  ...
                  >
                    ...
                    >
                      {current.content}
                      {current.shortcut ? (
                        <kbd className="warm-tooltip__kbd">
                          {current.shortcut}
                        </kbd>
                      ) : null}
                    </span>
