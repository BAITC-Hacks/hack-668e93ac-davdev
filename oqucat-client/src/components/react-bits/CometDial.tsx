...
const CometDial: React.FC<CometDialProps> = ({
  ...
  const paint = (now: number) => {
    ...
    if (g && g.at != null) {
      if (reduce) reading.jump(g.at)
        ...
        reading.set(
          reading.get() +
            (g.at - reading.get()) * (1 - Math.exp(-dt / TAU_FOLLOW))
        )
    }
    const r = reading.get()
    const vRaw =
      L.rPrev == null || dt === 0 || reduce ? 0 : (r - L.rPrev) / dt / range
    L.rPrev = r
    ...
    if (litRef.current)
      litRef.current.setAttribute('d', f > 0.0005 ? arcPath(start, ang) : '')
    if (headRef.current) {
    ...
    for (let j = 0; j < K; j++) {
      ...
      el.setAttribute(
        'stroke-width',
        (thickness + cometWidth * w * s).toFixed(2)
      )
  ...
  const localPoint = (cx: number, cy: number): [number, number] => {
    ...
    return [
      ((cx - b.left) / b.width) * 200 - 100,
      ((cy - b.top) / b.height) * 200 - 100,
    ]
  }
  ...
  const up = (e: PointerEvent) => {
    ...
    const bounce =
      tapBounce +
      (flickBounce - tapBounce) * clamp(Math.abs(v) / range / V_FLICK, 0, 1)
    const to = snap(
      (g.at ?? target.current) + (v / 1000) * (DECEL / (1 - DECEL)) * momentum
    )
    ...
    >
      ...
      >
        <span className="inline-flex items-baseline gap-[0.06em] leading-none font-medium tracking-[-0.04em] tabular-nums [font-size:var(--cd-figure)]">
          ...
          {unit ? (
            <span className="text-[0.5em] opacity-50">{unit}</span>
          ) : null}
        </span>
      </div>
    </div>
