...
const SplitFlapText = ({
  ...
  useEffect(() => {
    ...
    if (isFirstRender) {
      ...
      setTiles(createTiles(normalizePhrase(currentTextRef.current, width)))
    }
    ...
    const transitionDuration = isFirstRender ? 0 : animateTo(firstPhrase)

    if (normalizedPhrases.length > 1) {
      scheduleNext(safeCycleDelay + transitionDuration)
    }
