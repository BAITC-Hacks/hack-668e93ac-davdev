...
const StickerPeel: React.FC<StickerPeelProps> = ({
  ...
  useEffect(() => {
    ...
    const bounds = (target.parentNode as HTMLElement).getBoundingClientRect()
    const sticker = target.getBoundingClientRect()
    const maxX = Math.max(0, bounds.width - sticker.width)
    const maxY = Math.max(0, bounds.height - sticker.height)

    if (initialPosition === 'random') {
      startX = gsap.utils.random(0, maxX)
      startY = gsap.utils.random(0, maxY)
    }
    ...
    if (
      ...
      if (initialPosition.relative) {
        startX *= maxX
        startY *= maxY
      }
    }

    startX = gsap.utils.clamp(0, maxX, startX)
    startY = gsap.utils.clamp(0, maxY, startY)
