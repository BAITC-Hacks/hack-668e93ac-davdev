...
const TearTicket: React.FC<TearTicketProps> = ({
  ...
  const paint = (now: number): boolean => {
    ...
    c.geo.bridges.forEach((b, i) => {
      ...
      near.setAttribute(
        'd',
        `M${f(ox)},${f(oy)}L${f(ox + ux * len)},${f(oy + uy * len)}`
      )
      far.setAttribute(
        'd',
        `M${f(tx)},${f(ty)}L${f(tx - ux * len)},${f(ty - uy * len)}`
      )
  ...
  const step = (now: number) => {
    ...
    if (s.phase === 'held') {
      ...
      const up = c.geo.vertical
      const away = clamp(
        ((up ? s.point.y - s.start.y : s.point.x - s.start.x) || 0) * 0.05,
        -2,
        4
      )
      const side = clamp(
        ((up ? s.point.x - s.start.x : s.point.y - s.start.y) || 0) * 0.05,
        -3,
        3
      )
      ...
      c.geo.bridges.forEach((b, i) => {
        ...
        if (
          2 * d * Math.sin(s.theta / 2) + slack > c.stretch ||
          s.theta >= limit
        ) {
          s.snapped[i] = true
          s.snapAt[i] = now
  ...
  useEffect(() => {
    if (used) {
      const s = sim.current
      if (s.phase === 'idle' && stubRef.current)
        stubRef.current.style.visibility = 'hidden'
      return
    }
    setInstant(false)
    reset()
  ...
  const onStubDown = (e: PointerEvent<HTMLDivElement>) => {
    ...
    s.a0 =
      Math.atan2(s.grab.y - s.hinge.y, s.grab.x - s.hinge.x) -
      (s.theta * s.sign) / 0.92
    s.phase = 'held'
    s.thetaV = 0
    tiltX.set(0)
  ...
  const onStubMove = (e: PointerEvent<HTMLDivElement>) => {
    ...
    if (
      cfg.current.reduce &&
      Math.hypot(p.x - s.start.x, p.y - s.start.y) > 28
    ) {
      s.id = null
      setGrabbing(false)
      tearNow()
  ...
  useEffect(() => {
    ...
    const move = (e: globalThis.PointerEvent) => {
      ...
      const nx = clamp(
        (e.clientX - (r.left + r.width / 2)) / (r.width / 2 + tiltReach),
        -1,
        1
      )
      const ny = clamp(
        (e.clientY - (r.top + r.height / 2)) / (r.height / 2 + tiltReach),
        -1,
        1
      )
